"""CPU smoke validation for the maintained full-self-attention baselines."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run forward/backward CPU smoke checks for maintained transformer baselines."
    )
...
def main():
    ...
    checks = [
        ("ECalTransformer", ECalTransformer, ecal_transformer_view(event)),
        ("ECalTpadTransformer", ECalTpadTransformer, ecal_tpad_transformer_view(event)),
    ]
    print(f"target_mode: canonical-y target_label_order: {event['target_label_order']}")
    print(f"{'model':<21} {'input':<12} {'logits':<12} {'supervised':<12} {'loss':<10} status")
    for name, model_cls, view in checks:
        metrics = run_model_check(name, model_cls, view, args)
        print(
            f"{name:<21} {str(metrics['input_shape']):<12} {str(metrics['logits_shape']):<12} "
